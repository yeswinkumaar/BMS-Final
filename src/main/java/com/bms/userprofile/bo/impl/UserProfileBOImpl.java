...
public class UserProfileBOImpl implements UserProfileBO {
	...
	void postConstruct()
	{
		...
		CustomerCredentials customer=new CustomerCredentials("mohan123",UserProfileUtil.bcryptPasswordEncoder("yesH@123"),"cddsxssdsdd");
	...
	public UserProfileResponse updateUserProfile(UserProfileRequest userProfileRequest) throws  ParseException
	{
		...
		if(!(key==null))
		{
			...
			{
				if(!bmsValidator.isAlphaNumeric(userProfileRequest.getUserName(),false,8) || !bmsValidator.isNumeric(String.valueOf(userProfileRequest.getCustomerId()),false,8))
				{
				...
				}else
				{
					...
					if(userProfileRequest.getIsProfileChange().equals("Y"))
					{
						...
						updateStatus+="Customer Details Updated";
							...
							userProfileResponse.setHttpStatus(HttpStatus.OK);
							updateStatus+="Please enter some details";
						}
					}
					if(userProfileRequest.getIsAddressChange().equals("Y"))
					{
						
						String IsActive="Y",err;
						 err=userProfileValidation.addressValidation(userProfileRequest);
						...
						userProfileResponse.setHttpStatus(HttpStatus.OK);
							...
							updateStatus+="Please enter address";
							userProfileResponse.setHttpStatus(HttpStatus.OK);
						}
					}
	...
	public UserProfileResponse updateUserPassword(UserProfileRequest userProfileRequest)
	{
		...
		{
			System.out.println(customerCredential.getPassword()+userProfileRequest.getOldPassWord());
			userProfileResponse.setHttpStatus(HttpStatus.BAD_REQUEST);
		...
		else
		{
			...
			userProfileResponse.setHttpStatus(HttpStatus.OK);
			userProfileResponse.setResponse("Customer Password Updated");
		}
